...
from UdpEventThread import UdpEventThread
...
class ControllerThread(multiprocessing.Process):
    ...
    def run(self):
        ...
        self.connection = pymysql.connect(host='10.129.23.22',
                                     user='data_logging',
        ...
        print "Stopping UDP Threads"
        for t in self.udpThreads:
            self.udpThreads[t].stop()
            self.udpThreads[t].join()
    ...
    def dispatch_udp_task(self):
        sql1 = """select * from task_list t, meta_sensor s where type = 'U'
        ...
        for t in result:
            ...
            if t['type'] == 'U':
                flg_create = False
                # Check if the thread for the task is existing
                if t['data_source_id'] in self.udpThreads:
                    # Check if the task is running
                    if not self.udpThreads[t['data_source_id']].is_alive():
                        flg_create = True
                else:
                    flg_create = True
                # Start the process if required and or if the task as not running
                if flg_create:
                    # create process for the task
                    p = UdpEventThread(self.udp_q)
                    self.udpThreads[t['data_source_id']] = p
                    p.start()
                    flg_create = False
                # print "Put UDP Task",t
                self.udp_q.put(t)
